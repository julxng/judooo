...
import { ArtEvent, EventMedia } from '../types';
...
function getCoordinates(city: string, district: string): { lat: number; lng: number } {
...
function extractOrganizer(description: string, address: string): string {
...
function mapCategory(eventType: string, placeType: string): 'exhibition' | 'auction' | 'workshop' {
...
function transformEvent(raw: any, index: number): ArtEvent {
  ...
  return {
    ...
    description,
    category: mapCategory(raw.event_type || '', raw.place_type || '')
  };
}
